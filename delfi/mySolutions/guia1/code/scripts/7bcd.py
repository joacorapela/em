...
fig.add_trace(trace)
fig.update_xaxes(title="Distancia de la carga prueba al centro de la distribucion de cargas (m)")
# fig.update_yaxes(title=r"$\text{Norma 2 del Campo Eléctrico} (N/\mu C)$")
fig.update_yaxes(
    title_text=r"$\| \mathbf{E} \|_2 \quad [\text{N}/\mu\text{C}]$"
)
...
fig.add_trace(trace)
fig.update_xaxes(title="Distancia de la carga prueba al centro de la distribucion de cargas (m)")
fig.update_yaxes(title=r"$\frac{E_L-E_\infty}{E_\infty}$")
